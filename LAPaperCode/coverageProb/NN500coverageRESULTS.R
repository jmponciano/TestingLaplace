...
load("tmboptsNNdf3.RData")
load("NN500SIMSdf3.RData")


### Estimate coverage prob
df1 <- 3
parmsNN    <- list(beta=-1, df1=df1, sd1=3, sd2=2)
NSIMS      <- 500
...
for(i in 1:NSIMS){
  ...
  CI.mat.TMB[i,] <- c( parmsNN$beta, tmb.opts[[i]]$MLres$opt.ML$par, ci.obj )
}
...
load(paste0("/Users/constantingeorgeglen/Documents/Databases/Analyses/Program/TMB/code/LaplaceApproximation/Examples/coverageProb/",
            "optimBFGS_NN500covergedBASHdf3.RData"))

### Estimate coverage prob
parmsNN   <- list(beta=-1, df1=2, sd1=3, sd2=2)
NSIMS      <- 500
...
for(i in 1:NSIMS){
  ...
  CI.mat.MC[i,] <- c( parmsNN$beta, res[[i]]$obj$par, ci.obj )
}
...
summary(lm(tmbparhess~mcparhess, data=full.dataset%>% filter(tmbparhess<=210)))

### outlier
tmb.opts[[379.2075135]]$MLres$opt.ML
res[[379.2075135]]$obj

png( filename = "NNregress_hess.png", 
...
plot(mcparhess, tmbparhess, pch=16, cex=1.1,
     ...
     col="grey30", main = "Normal-Normal")
abline(reg = c(0,1), lty="dashed", lwd=1.5)
abline(reg = lm(tmbparhess~mcparhess, 
                data=full.dataset %>% filter(tmbparhess<=210)), 
       lty="solid", lwd=1.5)
...
summary(lm(tmbparinvhess~mcparinvhess, data=full.dataset %>% filter(tmbparinvhess<=0.3)))


png( filename = "NNregress_invhess.png", 
     width = 12, height = 8, units = "in", res = 1200, pointsize = 10
)
par(mar=c(5,5,2,3), mfrow=c(1,1))
plot(mcparinvhess, tmbparinvhess, pch=16, 
     ...
     col="grey30", main = "Normal-Normal")
abline(reg = c(0,1), lty="dashed", lwd=1.5)
abline(reg = lm(tmbparinvhess~mcparinvhess, 
                data=full.dataset %>% filter(tmbparinvhess<=0.3)), 
       lty="solid", lwd=1.5)
# points(parmsNN$beta,parmsNN$beta, pch=15, cex=1.2)
dev.off()
...
png( filename = "NNregress_est.png", 
     width = 12, height = 8, units = "in", res = 1200, pointsize = 10
     )
par(mar=c(5,5,2,3), mfrow=c(1,1))
plot(mcparest, tmbparest, pch=16, cex=1.1, 
     xlim=c(-2,0.5), ylim=c(-2,0.5),
     cex.axis=1.5, cex.lab=1.5, cex.main=2,
     xlab=expression(MC ~ beta), ylab=expression(TMB ~ beta), bty="l",
     col="grey30", main = "Normal-Normal")
abline(reg = c(0,1), lty="dashed", lwd=1.5)
abline(reg = lm(tmbparest~mcparest, 
                data=full.dataset %>% filter(tmbparest<=0.5 & tmbparest >=-2)), 
       lty="solid", lwd=1.5)
# points(parmsNN$beta,parmsNN$beta, pch=15, cex=1.2)
dev.off()
...
plot( c(0,0), type="n", xlim=c(range(CI.mat.TMB[,3:4])), ylim=c(1,NSIMS),
      ...
      main=paste0("Normal-Normal TMB coverage: ",coverage.TMB) )
for(i in 1:NSIMS){
...
abline(v=parmsNN$beta, lwd=1, lty="dashed", col="black")
...
plot( c(0,0), type="n", xlim=c(range(CI.mat.MC[,3:4])), ylim=c(1,NSIMS),
      ...
      cex.axis=1.5, cex.lab=1.5, cex.main=1.5,
      main=paste0("Normal-Normal MC coverage: ",coverage.MC))
for(i in 1:NSIMS){
