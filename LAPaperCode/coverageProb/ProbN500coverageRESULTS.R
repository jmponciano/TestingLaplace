...
for(i in 1:NSIMS){
  ...
  CI.mat.TMB[i,] <- c( parmsProbN$beta, tmb.opts[[i]]$MLres$opt.ML$par, ci.obj )
}
...
#### MC results
library(Hmisc)
load("/Users/constantingeorgeglen/Documents/Databases/Analyses/Program/TMB/code/LaplaceApproximation/Examples/coverageProb/optimBFGS_ProbN500covergedBASH.RData")
# load("/Users/constantingeorgeglen/Documents/Databases/Analyses/Program/TMB/code/LaplaceApproximation/Examples/coverageProb/optimNelderM_ProbN500covergedBASH.RData")
...
for(i in 1:NSIMS){
  ...
  CI.mat.MC[i,] <- c( parmsProbN$beta, res[[i]]$obj$par, ci.obj )
}
...
png( filename = "ProbNregress_hess.png", 
...
png( filename = "ProbNregress_invhess.png", 
     width = 12, height = 8, units = "in", res = 1200, pointsize = 10
)
par(mar=c(5,5,2,3), mfrow=c(1,1))
...
png( filename = "ProbNregress_est.png", 
     width = 12, height = 8, units = "in", res = 1200, pointsize = 10
)
par(mar=c(5,5,2,3), mfrow=c(1,1))
plot(mcparest, tmbparest, pch=16, cex=1.1, 
     xlim=c(-15,1), ylim=c(-2,1),
     cex.axis=1.5, cex.lab=1.5, cex.main=2,
     xlab=expression(MC ~ beta), ylab=expression(TMB ~ beta), bty="l",
     col="grey30", main = "Probit-Normal")
# abline(reg = c(0,1), lty="dashed", lwd=1.5)
# abline(reg = lm(tmbparest~mcparest), lty="dashed", lwd=1.5)
# points(parmsNN$beta,parmsNN$beta, pch=15, cex=1.2)
dev.off()
...
### Coverage plot
png( filename = "ProbNcoverage.png",
     width = 12, height = 8, units = "in", res = 1200, pointsize = 10
)
...
plot( c(0,0), type="n", xlim=c(range(CI.mat.TMB[,3:4])), ylim=c(1,NSIMS),
      ...
      main=paste0("Probit-Normal TMB coverage: ",round(coverage.MC,3)))
for(i in 1:NSIMS){
...
abline(v=parmsProbN$beta, lwd=1, lty="dashed", col="black")
...
plot( c(0,0), type="n", 
      ...
      cex.axis=1.5, cex.lab=1.5, cex.main=1.5,
      main=paste0("Probit-Normal MC coverage: ", round(coverage.MC,3)))
for(i in 1:NSIMS){
