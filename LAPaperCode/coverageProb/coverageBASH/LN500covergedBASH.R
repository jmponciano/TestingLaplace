##### ---------------------------------------------------------------
##### Coverage probability for Logit-Normal model
...
negll <- function(betaguess,W,Y, sd1=sd1,X.B=X.B){
  ...
  for(i in 1:n){
    ...
    Y.like <- dbinom(rep(Y[i],B), size=1, prob=inv.logit(beta*X.B))
...
write.csv(matrix(data=NA),  file=paste0(wd.path.name,"/testLN.csv"))

### Procedure section
cl <- makeCluster(15)
registerDoParallel(cl)
len.LN <- length(LN.SIMS)
res <- foreach(i=1:len.LN, .export = ls(.GlobalEnv)) %dopar%{
  
  temp.data <- LN.SIMS[[i]]
  inv.logit <- boot::inv.logit
  
  # First we simulate a large number of samples from the t distrib
  parms.LN <- list(beta=-1, df1=10, sd1=3)
  B           <- 500000
  X.B <- rt(n=B*length(temp.data[,2]), df=parms.LN$df1)
  out <- optim(par=-1, fn=negll,
               method="Nelder-Mead", 
               # method="BFGS", 
               W=temp.data[,1], Y=temp.data[,2],
               sd1=parms.LN$sd1, X.B=X.B, hessian=TRUE)
...
### SAVE the RDATA
save.image(file = paste0(wd.path.name,"/LN500covergedBASH.RData"))
