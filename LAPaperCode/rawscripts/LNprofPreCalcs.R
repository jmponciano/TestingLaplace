...
check.model
#sqrt(diag(solve(ModLN.TMB$model.obj$hessian)))


# manual computing sd
import::from("numDeriv", "hessian")
(std <- sqrt(diag(solve(numDeriv::hessian(ModLN.TMB$fun.obj$fn, ModLN.TMB$fun.obj$env$last.par.best[1:2])))))
(std <- sqrt(diag(solve(numDeriv::jacobian(ModLN.TMB$fun.obj$gr,ModLN.TMB$fun.obj$env$last.par.best[1:2])))))
par.fixed <-ModLN.TMB$fun.obj$env$last.par.best[1:2]
hessian.fixed <- optimHess(par.fixed, ModLN.TMB$fun.obj$fn,
                           ModLN.TMB$fun.obj$gr) ## Marginal precision of theta.
chol(hessian.fixed) # pdHess
ModLN.TMB$model.obj$hessian
